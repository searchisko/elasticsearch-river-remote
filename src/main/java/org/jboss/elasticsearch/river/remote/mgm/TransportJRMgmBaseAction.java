...
/**
 * Base abstract transport action for Remote River management operations.
 * 
...
public abstract class TransportJRMgmBaseAction<Request extends JRMgmBaseRequest, Response extends JRMgmBaseResponse, NodeRequest extends NodeJRMgmBaseRequest<Request>, NodeResponse extends NodeJRMgmBaseResponse>
	...
	protected NodeResponse nodeOperation(NodeRequest nodeRequest) throws ElasticsearchException {
		...
		IRiverMgm river = RemoteRiver.getRunningInstance(req.getRiverName());
		if (river == null) {
			logger.debug("River {} not found on this node", req.getRiverName());
	...
	/**
	 * Implement in subclass to perform necessary management operation on the river instance. This method is called only
